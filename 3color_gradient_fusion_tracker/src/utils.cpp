...
cv::Mat returnColorHistogramAroundRegion(Mat cropped_frame, int hist_size) {
	Mat tempHistogram;
	Mat basicModelHistogram;
	float range[] = { 0, 256 };
	const float* histRange = { range };

	calcHist(&cropped_frame, 1, 0, Mat(), tempHistogram, 1, &hist_size,
			&histRange, true, false);
	normalize(tempHistogram, basicModelHistogram, 0, 1, NORM_MINMAX, -1, Mat());

	return basicModelHistogram;
}
cv::Mat returnGradientHistogramAroundRegion(Mat cropped_frame, int hist_size) {
...
void calculateCandidates(const Mat singleChannelFrame,
		std::vector<Mat> &candidates, std::vector<Rect> &candidate_rectangle,
		Rect currentState, int size_of_neighbourhood, int step_size,
		int hist_size, bool color_based) {
...
//check if our window wont be out of the frame
	...
	if (currentState.x + size_of_neighbourhood < singleChannelFrame.cols) {
		endx = currentState.x + size_of_neighbourhood;
	} else {
		endx = singleChannelFrame.cols;
	}
	if (currentState.y + size_of_neighbourhood < singleChannelFrame.rows) {
		endy = currentState.y + size_of_neighbourhood;
	} else {
		endy = singleChannelFrame.rows;
	}
...
//iterate over all the possible candidate places, where i and j are the coordinates of the centers of the potential candidates
	for (int i = startx; i < endx; i += step_size) {
		for (int j = starty; j < endy; j += step_size) {
			...
			if (i > 0 && (i + currentState.width) < singleChannelFrame.cols
					&& j > 0
					&& (j + currentState.height) < singleChannelFrame.rows) {
				Mat singleCandidate;
				Mat cropped;
//frame.copyTo(cropped);
				cropped = singleChannelFrame(
						Rect(i, j, currentState.width, currentState.height));

				if (color_based) {
					singleCandidate = returnColorHistogramAroundRegion(cropped,
							hist_size);
				} else {
					singleCandidate = returnGradientHistogramAroundRegion(
							cropped, hist_size);
				}

				candidates.push_back(singleCandidate);
				candidate_rectangle.push_back(
						Rect(i, j, currentState.width, currentState.height));
			}
		}
	}
...
cv::Mat histogramImage(Mat basicModelHistogram, Mat currentHistogram,
		int hist_size) {
	...
	normalize(basicModelHistogram, modelNormalizedHistogram, 0, histImage.rows,
			NORM_MINMAX, -1, Mat());
	normalize(currentHistogram, winnerHistogram, 0, histImage.rows, NORM_MINMAX,
			-1, Mat());
	// Draw for each channel
	for (int i = 0; i < hist_size; i++) {
		line(histImage,
				Point(bin_w * (i - 1),
						hist_h - cvRound(winnerHistogram.at<float>(i - 1))),
				Point(bin_w * (i),
						hist_h - cvRound(winnerHistogram.at<float>(i))),
				Scalar(0, 255, 0), 2, 8, 0);
		line(histImage,
				Point(bin_w * (i - 1),
						hist_h
								- cvRound(
										modelNormalizedHistogram.at<float>(
												i - 1))),
				Point(bin_w * (i),
						hist_h
								- cvRound(
										modelNormalizedHistogram.at<float>(i))),
...
void CalculateDIstances(const Mat basicModelHistogram,
		const std::vector<Mat> candidates, std::vector<float> &distances,
		bool color_based) {
	//Compare object and its candidates:
	if (color_based) {
		std::cout << candidates.size() << std::endl;
		distances.push_back(
				compareHist(basicModelHistogram, candidates[0],
						CV_COMP_BHATTACHARYYA));
	} else {
		distances.push_back(norm(basicModelHistogram, candidates[0], NORM_L2));
	...
	for (int i = 1; i < candidates.size(); i++) {
		if (color_based) {
			distances.push_back(
					compareHist(basicModelHistogram, candidates[i],
							CV_COMP_BHATTACHARYYA));
		} else {
			distances.push_back(
					norm(basicModelHistogram, candidates[i], NORM_L2));
		}
	}
	if (color_based) {
		//cout<< "BHATTACHARYYA distance of color histogram"<< smallest_dist <<endl;
	} else {
		//cout<< "L2 distance of HOG:  "<< smallest_dist <<endl;
	}
	normalize(distances, distances, 0, 1, NORM_MINMAX, -1, Mat());
}

int selectCombinedCandidate(std::vector<float> distances_color,
		std::vector<float> distances_gradient, bool only_color,
		bool only_gradient) {
	//Compare object and its candidates:
	...
	if (only_color) {
		smallest_dist = distances_color[0];
	} else if (only_gradient) {
		smallest_dist = distances_gradient[0];
	} else {
		smallest_dist = distances_color[0] + distances_gradient[0];
	}

	for (int i = 0; i < distances_color.size(); i++) {
		if (only_color) {
			current_distance = distances_color[i];
		} else if (only_gradient) {
			current_distance = distances_gradient[i];
		} else {
	...
	//cout<< "the chosen color distance: "<<distances_color[index_with_smallest_distance]<<endl;
	//cout<< "the chosen gradient distance: "<<distances_gradient[index_with_smallest_distance]<<endl;
	cout << "the combined distance:  " << smallest_dist << endl;
